...
const addMoviesToDom = function (films) {
    ...
    films.forEach(function (film) {
        ...
        newA.setAttribute('href', 'https://www.imdb.com/title/' + film.imdbID); //attribute for a-tags (href) + external link to imdb 
...
const searchMovie = function () {
    const searchField = document.querySelector('.searchForm'); //select input field
    const searchButton = document.querySelector('.searchButton'); //select search-button

    searchButton.addEventListener('click', function () {
        const filteredMovies = movies.filter(function (movie) {
            const movieTitleContainsSearchText = movie.Title.includes(searchField.value);
